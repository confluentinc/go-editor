...
LATEST_VERSION := $(shell git rev-parse --is-inside-work-tree > /dev/null && git ls-remote --tags --refs --sort="v:refname" | \
tail -n1 | tr -d " \t\n\r" | cut -d'/' -f3)
VERSION_SUFFIX := $(shell git rev-parse --is-inside-work-tree > /dev/null && git describe --tags --always --dirty | cut -d'-' -s -f2,3,4)
