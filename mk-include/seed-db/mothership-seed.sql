...
CREATE VIEW deployment.logical_network_access AS
 SELECT lc.id AS id
   , lc.type AS type
   , lc.deactivated AS deactivated
   , net_access.network_access
   , d.network_region_id AS network_region_id
   , pc.id AS physical_cluster_id
   , pc.config->'kafka'->>'kafka_api_id' AS pkac_id
   , pc.k8s_cluster_id AS k8s_cluster_id
   , lc.modified AS modified
   , lc.last_change_id AS change_id
 ...
 -- using LATERAL instead of GROUP BY b/c it's less verbose and easier to read
 LEFT JOIN LATERAL(
    -- aggregate network access configs per type for each deployment
    WITH na_by_type AS (
      SELECT na.type, jsonb_agg(na.config) AS configs
      FROM deployment.network_access AS na
      WHERE na.deployment_id = d.id AND na.deactivated IS NULL
      GROUP by na.type
    )
    -- combine the access config lists per type into an object per deployment
    SELECT jsonb_object_agg(na_by_type.type, na_by_type.configs) AS network_access
    FROM na_by_type
 ) AS net_access ON TRUE
